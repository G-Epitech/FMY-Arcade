...
std::vector<EventPtr> EventsHandler::handleEvents() {
    ...
    // sf::Event NCURSESEvent{};

    // while (_window.getInnerWindow().pollEvent(NCURSESEvent)) {
    //     auto handler = _getHandler(NCURSESEvent.type);
    //     auto event = handler ? handler(NCURSESEvent, _window) : nullptr;
